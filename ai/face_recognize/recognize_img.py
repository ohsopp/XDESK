...
def tracking_img(detector, test_path, args):
    """ 
        tracking image from folder
    """
    ...
    for image_name in os.listdir(test_path):
        if image_name.endswith(("jpg")):
            ...
            tracking_image = process_tracking(img, detector, tracker, args, 0, -1)
            result.append(recognize_img(0.25))
    return result;
...
def main():
    read_features("./datasets/face_features/feature")
    file_name = "./face_tracking/config/config_tracking.yaml"
    test_path = "./test"
    config_tracking = load_config(file_name)

    images_names, images_embs = read_features(feature_path="./datasets/face_features/feature")
    print(tracking_img(detector, test_path, config_tracking))
